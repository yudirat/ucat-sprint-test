...
export const DataSourceRenderer: React.FC<DataSourceRendererProps> = ({ type, dataSource, currentQuestionId }) => {
  ...
  if (dataSource.isSprint && currentQuestionId !== undefined) {
    ...
    // Sprint A (1-18): 1-5 (Table), 6-9 (Pie), 10-14 (Bar), 15-18 (Line)
    // Sprint B (19-36): 19-27 (Multi), 28-36 (Table)
    const scenario = dataSource.scenarios.find((s: any) => {
      // This is a bit of a hack, in a real app we'd store question ranges in the scenario object
      if (currentQuestionId <= 5) return s.id === 1;
      if (currentQuestionId <= 9) return s.id === 2;
      if (currentQuestionId <= 14) return s.id === 3;
      if (currentQuestionId <= 18) return s.id === 4;
      // Sprint B (19-36)
      if (currentQuestionId <= 27) return s.id === 1;
      if (currentQuestionId <= 36) return s.id === 2;
      // Sprint C - VR (37-58)
      if (currentQuestionId <= 40) return s.id === 1;
      if (currentQuestionId <= 44) return s.id === 2;
      if (currentQuestionId <= 48) return s.id === 3;
      if (currentQuestionId <= 53) return s.id === 4;
      if (currentQuestionId <= 58) return s.id === 5;
      // Sprint D - DM (59-76)
      if (currentQuestionId <= 63) return s.id === 1;
      if (currentQuestionId <= 68) return s.id === 2;
      if (currentQuestionId <= 72) return s.id === 3;
      if (currentQuestionId <= 76) return s.id === 4;
      return false;
    });
    ...
    return (
      <div className="space-y-0 divide-y divide-slate-200">
        <div className="p-6 bg-blue-50/50">
          <h4 className="font-black text-[#004a99] text-xs uppercase mb-4 tracking-widest">
            Energy Production Breakdown (Total: {activeData.pieTotal} TWh)
          </h4>
        ...
        <table className="w-full text-sm">
          <thead className="bg-[#004a99] text-white">
            <tr>
              <th className="px-4 py-2 text-left text-xs font-bold uppercase">Renewable Source</th>
              <th className="px-4 py-2 text-left text-xs font-bold uppercase">% of Renewables</th>
              <th className="px-4 py-2 text-left text-xs font-bold uppercase">Cost/TWh (£m)</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200">
  ...
  if (activeType === 'LINE_GRAPH') {
    return (
      <div className="p-6">
        <h4 className="font-black text-[#004a99] text-xs uppercase mb-4 tracking-widest">Data Trends</h4>
        <table className="w-full text-xs border border-slate-300 border-collapse">
          <thead>
            <tr className="bg-slate-800 text-white">
              <th className="border border-slate-600 p-2">Marker</th>
              {activeData.timeHours.map((h: number) => <th key={h} className="border border-slate-600 p-2">{h}</th>)}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200">
            <tr className="bg-white">
              <td className="border border-slate-200 p-2 font-bold bg-slate-100">Index A</td>
              {activeData.vanA_Dist.map((d: number, i: number) => <td key={i} className="border border-slate-200 p-2 text-center">{d}</td>)}
            </tr>
            <tr className="bg-slate-50">
              <td className="border border-slate-200 p-2 font-bold bg-slate-100">Index B</td>
    ...
    return (
      <div className="space-y-0 divide-y divide-slate-200">
        <table className="w-full text-sm">
          <thead className="bg-[#004a99] text-white">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-bold uppercase">Cost (Δ)</th>
              <th className="px-4 py-3 text-left text-xs font-bold uppercase">Maint.</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200">
        ...
        <div className="p-6 bg-slate-50">
          <h4 className="font-bold text-xs uppercase text-slate-500 mb-4 tracking-wider">Exchange Rates (1 Unit Δ =)</h4>
          <div className="flex gap-12">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-400 font-bold">GBP (£)</span>
              <span className="text-xl font-black text-[#004a99]">{activeData.exchange.GBP}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-400 font-bold">USD ($)</span>
              <span className="text-xl font-black text-[#004a99]">{activeData.exchange.USD}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  if (activeType === 'PIE_CHART') {
    return (
      <div className="p-8 flex flex-col items-center">
        <h4 className="font-black text-[#004a99] text-xs uppercase mb-8 tracking-widest text-center">{activeData.title}</h4>
        ...
        <p className="mt-8 text-xs font-bold text-slate-400">Total Base: {activeData.total}</p>
      </div>
    );
  }
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="border-2 border-[#004a99] p-4 rounded-lg bg-blue-50">
            ...
            <div className="aspect-square bg-white border border-slate-300 flex items-center justify-center text-xs text-slate-400 italic">
              [Set A Image Placeholders]
            </div>
  ...
  if (activeType === 'PASSAGE') {
    return (
      <div className="p-8 prose prose-slate max-w-none">
        <h4 className="font-black text-[#004a99] text-xs uppercase mb-6 tracking-widest">{activeData.title}</h4>
  ...
  if (activeType === 'TEXT_LOGIC') {
    return (
      <div className="p-8 space-y-4">
        <h4 className="font-bold text-slate-500 uppercase text-xs tracking-widest">Logic Premises</h4>
  ...
  if (activeType === 'VENN_DIAGRAM') {
    return (
      <div className="p-4 flex flex-col items-center">
        <h4 className="font-black text-[#004a99] text-xs uppercase mb-4 tracking-widest text-center">{activeData.title}</h4>
